// ---------- THEME PICKER ----------
const THEMES = [
  { id: 'night', name: 'Night', tag: 'Oiled leather, copper ink',  swatch: ['#1a140e', '#e87c3a', '#f3e5c2'] },
  { id: 'day',   name: 'Day',   tag: 'Aged paper, red pen accents', swatch: ['#f1ebd9', '#b8410e', '#1c1812'] }
];
...
function renderThemeToggle() {
  ...
  if (!btn) return;
  // Two SVGs (sun + moon) live inside the button; CSS shows the right one via [data-theme] on <html>.
  // We just keep the aria-label / title in sync so screen readers know what'll happen.
  const cur = getTheme();
